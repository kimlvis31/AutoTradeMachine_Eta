...
_BINANCE_TWM_STREAMRENEWALPERIOD_S            = 60*240 #Every 240 Minutes (4 Hours)
...
class BinanceAPIManager:
            ...
            for target in ('kline', 'depth', 'aggTrade', 'metric'):
                ...
                #[2-4]: If Waiting GBVF
                    ...
                    if not data:
                        ...
                        if data is None: request['_waitUntil']  = func_gnitt(intervalID = auxiliaries.KLINE_INTERVAL_ID_1m, timestamp = t_current_s, nTicks = 1)
                        else:            request['_waitUntil']  = func_gnitt(intervalID = auxiliaries.KLINE_INTERVAL_ID_1h, timestamp = t_current_s, nTicks = 1)
                        symbols_processed.append((symbol, target, False))
                        continue
    ...
    def __searchKlineFirstOpenTS(self, symbol):
        ...
        ts_target_end = func_gnitt(intervalID = auxiliaries.KLINE_INTERVAL_ID_1M, 
        ...
        ts_target_end = func_gnitt(intervalID = auxiliaries.KLINE_INTERVAL_ID_1h, 
                                   timestamp  = ts_target_beg, 
                                   nTicks     = 24)-1
        try: fetchedKlines = func_fhk(symbol    = symbol, 
    ...
    def __generateFetchTasks(self, target, visionFiles, fetchTargetRangeType, fetchTargetRanges):
        ...
        func_gnitt  = auxiliaries.getNextIntervalTickTimestamp
        interval_1d = auxiliaries.KLINE_INTERVAL_ID_1d
        visionFileRanges = []
    ...
    def __handleFetchTask_REST_kline(self, symbol, cause, fetchTask, isLastCompletion, requestID):
        ...
        if fetchTask['status'] == 'pending':
            ...
            ftr_end_max = auxiliaries.getNextIntervalTickTimestamp(intervalID = KLINTERVAL, 
                                                                          timestamp  = ftr[0], 
                                                                          mrktReg    = None,
                                                                          nTicks     = 1000)-1
        ...
        if fetchTask['status'] == 'fetched':
            ...
            efkts_expected = auxiliaries.getTimestampList_byRange(intervalID        = KLINTERVAL, 
                                                                         mrktReg           = None, 
        ...
        if task_status == 'pending':
            ...
            try: 
                ...
                if ft_type == 'timestampBeg_aggTIDEnd':
                    ...
                    ts_end_ms = func_gnitt(intervalID = auxiliaries.KLINE_INTERVAL_ID_1h, timestamp = ts_beg_ms, mrktReg = None, nTicks = 1)*1000-1
        ...
        if fetchTask['status'] == 'fetched':
            ...
            if isinstance(fetchedDepths, bytes):
                ...
                if KLINTERVAL == '1M':
                    ...
                    interval_sec = auxiliaries.KLINE_INTERVAL_SECs[KLINTERVAL]
    ...
    def __handleFetchTask_VISION_aggTrade(self, symbol, cause, fetchTask, isLastCompletion, requestID):
        ...
        if fetchTask['status'] == 'fetched':
            ...
            fetchTask['data'] = []
            for fetchedRange in fetchedRanges:
                efatts = auxiliaries.getTimestampList_byRange(intervalID        = KLINTERVAL, 
                                                                     mrktReg           = None, 
                                                                     timestamp_beg     = fetchedRange[0], 
                                                                     timestamp_end     = fetchedRange[1], 
                                                                     lastTickInclusive = True)
