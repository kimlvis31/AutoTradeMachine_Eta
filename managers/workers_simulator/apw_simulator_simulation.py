...
class Simulation:
    ...
    def __init__(self, path_project, simulatorIndex, ipcA, simulationCode, simulationRange, analysisExport, assets, positions, currencyAnalysisConfigurations, tradeConfigurations, creationTime):
        ...
        self.__simulationRange                = (atmEta_Auxillaries.getNextIntervalTickTimestamp(intervalID = KLINTERVAL, timestamp = simulationRange[0], nTicks = 0),
                                                 atmEta_Auxillaries.getNextIntervalTickTimestamp(intervalID = KLINTERVAL, timestamp = simulationRange[1], nTicks = 1)-1)
        ...
        self.__lastAnalysisTarget = atmEta_Auxillaries.getNextIntervalTickTimestamp(intervalID = KLINTERVAL, timestamp = simulationRange[1], nTicks = 0)
    ...
    def __computeLiquidationPrice(self, positionSymbol, walletBalance, quantity, entryPrice, currentPrice, maintenanceMargin, upnl, isolated = True, mm_crossTotal = 0, upnl_crossTotal = 0):
        ...
        maintenanceMarginRate, maintenanceAmount = atmEta_Constants.getMaintenanceMarginRateAndAmount(positionSymbol = positionSymbol, notional = quantity_abs*currentPrice)
        if isolated:
            mm_others   = 0
            upnl_others = 0
        else:
