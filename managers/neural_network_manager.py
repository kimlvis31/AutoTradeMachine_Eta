...
_NUMPYDTYPE = neural_networks._NUMPYDTYPE
_TORCHDTYPE = neural_networks._TORCHDTYPE
...
class NeuralNetworkManager:
    ...
    def __generateNeuralNetwork(self, neuralNetworkCode, neuralNetworkType, initialization, nKlines, hiddenLayers, outputLayer, controlKey):
        ...
        try:
            if (neuralNetworkType == 'MLP'): _neuralNetworkInstance = neural_networks.neuralNetwork_MLP(nKlines = nKlines, hiddenLayers = hiddenLayers, outputLayer = outputLayer, initialization = initialization)
    ...
    def __importNeuralNetwork(self, neuralNetworkCode, neuralNetworkDescription):
        ...
        _completion          = None
        _completion_ETC_s    = None
        if (_neuralNetworkType == 'MLP'): _neuralNetworkInstance = neural_networks.neuralNetwork_MLP(nKlines = _nKlines, hiddenLayers = _hiddenLayers, outputLayer = _outputLayer)
            ...
            if ((self.__neuralNetwork_currentProcessInternalData != None) and (requestID == self.__neuralNetwork_currentProcessInternalData['klineFetchRequestRID'])):
                ...
                if (_rr_result == 'SKF'):
                    ...
                    for _index, _kline in enumerate(_rr_klines): 
                        ...
                        _t_open_prev = auxiliaries.getNextIntervalTickTimestamp(intervalID = KLINTERVAL, timestamp = _t_open, mrktReg = None, nTicks = -1)
