...
class Account:
    ...
    def __update(self, assets, positions):
        ...
        for assetName, asset_ip in assets_ip.items():
            ...
            asset['allocatableBalance']            = round(max(0.0, asset['walletBalance'])*_ACCOUNT_BASEASSETALLOCATABLERATIO*asset['allocationRatio'], _ACCOUNT_ASSETPRECISIONS[assetName])
    ...
    def __handleAnalysisResult(self, symbol, linearizedAnalysis):
        ...
        for thType in tradeHandlers:
            ...
            th_new = True
        if th_new:
            position['_tradeHandlers_genCounter'] += 1
    ...
    def __processTradeHandlers(self):
        ...
        for symbol, position in self.__positions.items():
            ...
            if not tradeHandlers: 
                continue
            #---[2-2-1]: Trade Handlers Preceding The Latest FSL Removal
            idx_fsl = None
            for idx, th_q in enumerate(tradeHandlers):
                if th_q['id'] is None: 
                    idx_fsl = idx
            if idx_fsl is not None:
                for _ in range(idx_fsl): 
                    tradeHandlers.popleft()
            #---[2-2-2]: OCR Check
            ocr = position['_orderCreationRequest']
            if ocr is not None:
                ocr_status  = ocr['status']
                ocr_thID    = ocr['tradeHandlerID']
                th_hasFSL   = (tradeHandlers[0]['id'] is None)
                th_newestID = tradeHandlers[-1]['id']
                if (ocr_status == 'RESTING' and                                           #Is this OCR at a RESTING status?
                    ocr_thID is not None    and                                           #Was this OCR generated by a TEF trade handler?
                    (th_hasFSL or (th_newestID is not None and ocr_thID < th_newestID))): #Is there an FSL trade handler or a newer tick handler?
                    ...
                    self.__logger(message = (f"OCR Canceling For {self.__localID}-{symbol} By Trade Handler.\n"
                                  ...
                                  color   = 'light_green')
                continue
            #---[2-2-3]: Position Identity Control Requests Check
            ...
            #---[2-5-1]: ENTRY
            if th_type == 'ENTRY':
                #[2-5-1-1]: Post Stop Loss Reentry Check
    ...
    def __trade_checkConditionalExits(self, symbol, kline):
        ...
        tradeHandler = None
        if   tradeHandler_checkList['FSLIMMED'] is not None: tradeHandler = 'FSLIMMED'
        elif tradeHandler_checkList['FSLCLOSE'] is not None: tradeHandler = 'FSLCLOSE'

        #[5]: Finally
        position_ths = position['_tradeHandlers']
        if tradeHandler is not None:
            #[5-1]: Add Trade Handler
            th = {'type':              tradeHandler, 
                  'side':              tradeHandler_checkList[tradeHandler],
                  'tefVal':            None,
