...
EXPECTEDTEMPORALWIDTHS = constants.EXPECTEDTEMPORALWIDTHS
...
class chartDrawer:
    ...
    def __init__(self, **kwargs):
        ...
        for amType, am in analyzers.ANALYSES.items():
            ...
            self.__onPSUs[amType] = am['FN_CD_PSU']
        ...
        for amType, am in analyzers.ANALYSES.items():
            if am['TYPE'] != 'SUB':
                continue
            self.checkVerticalExtremas_SIs[amType] = am['FN_CD_CVE']
        ...
        self.intervalID     = constants.KLINE_INTERVAL_ID_1m
        #---Data
            ...
            if onInit:
                for displayBoxName in self.displayBox:
                    ...
                    #---KLINESPRICE
                        ...
                        self.displayBox_graphics['KLINESPRICE']['HORIZONTALGUIDELINE'] = pyglet.shapes.Line(0, 0, drawBox[2]*self.scaler, 0, width = 3, color = self.guideColor, batch = self.batch, group = self.displayBox_graphics['KLINESPRICE']['DESCRIPTORDISPLAY_CAMGROUP'])
        ...
        if siViewerDisplay1:
            ...
            if self.siTypes_analysisCodes[siViewerDisplayTarget1] is not None:
                for aCode in self.siTypes_analysisCodes[siViewerDisplayTarget1]:
                    self.addBufferZone_toDrawQueue(analysisCode = aCode, 
                                                   drawSignal   = _FULLDRAWSIGNALS[siViewerDisplayTarget1])
    ...
    def _initializeSIViewer(self, siViewerCode):
        ...
        if siAlloc in _VVR_DEFAULT: vvr_default = _VVR_DEFAULT[siAlloc]
        else:                       vvr_default = _VVR_DEFAULT[(siAlloc, oc[f'{siAlloc}_DisplayType'])]
    ...
    def __initializeAuxBar(self):
        ...
        aggIntervals = [(KLINE_INTERVAL_ID_1m,  'GUIO_CHARTDRAWER:AGGINTERVAL_1M'),
                        (KLINE_INTERVAL_ID_3m,  'GUIO_CHARTDRAWER:AGGINTERVAL_3M'),
                        (KLINE_INTERVAL_ID_5m,  'GUIO_CHARTDRAWER:AGGINTERVAL_5M'),
                        (KLINE_INTERVAL_ID_15m, 'GUIO_CHARTDRAWER:AGGINTERVAL_15M'),
                        (KLINE_INTERVAL_ID_30m, 'GUIO_CHARTDRAWER:AGGINTERVAL_30M'),
                        (KLINE_INTERVAL_ID_1h,  'GUIO_CHARTDRAWER:AGGINTERVAL_1H'),
                        (KLINE_INTERVAL_ID_2h,  'GUIO_CHARTDRAWER:AGGINTERVAL_2H'),
                        (KLINE_INTERVAL_ID_4h,  'GUIO_CHARTDRAWER:AGGINTERVAL_4H'),
                        (KLINE_INTERVAL_ID_6h,  'GUIO_CHARTDRAWER:AGGINTERVAL_6H'),
                        (KLINE_INTERVAL_ID_8h,  'GUIO_CHARTDRAWER:AGGINTERVAL_8H'),
                        (KLINE_INTERVAL_ID_12h, 'GUIO_CHARTDRAWER:AGGINTERVAL_12H'),
                        (KLINE_INTERVAL_ID_1d,  'GUIO_CHARTDRAWER:AGGINTERVAL_1D'),
                        (KLINE_INTERVAL_ID_3d,  'GUIO_CHARTDRAWER:AGGINTERVAL_3D'),
                        (KLINE_INTERVAL_ID_1W,  'GUIO_CHARTDRAWER:AGGINTERVAL_1W'),
                        (KLINE_INTERVAL_ID_1M,  'GUIO_CHARTDRAWER:AGGINTERVAL_1MONTH')]
    ...
    def __initializeSettingsSubPage(self):
        ...
        yPos_beg              = 20000
        guioTypes = {'passiveGraphics_wrapperTypeB': generals.passiveGraphics_wrapperTypeB,
                     'passiveGraphics_wrapperTypeC': generals.passiveGraphics_wrapperTypeC,
                     'textBox_typeA':                generals.textBox_typeA,
                     'imageBox_typeA':               generals.imageBox_typeA,
                     'button_typeA':                 generals.button_typeA,
                     'button_typeB':                 generals.button_typeB,
                     'switch_typeB':                 generals.switch_typeB,
                     'switch_typeC':                 generals.switch_typeC,
                     'slider_typeA':                 generals.slider_typeA,
                     'textInputBox_typeA':           generals.textInputBox_typeA,
                     'LED_typeA':                    generals.LED_typeA,
                     'selectionBox_typeB':           generals.selectionBox_typeB,
                     'selectionBox_typeC':           generals.selectionBox_typeC,
                     'subPageBox_typeA':             generals.subPageBox_typeA}
        ...
        for amType, am in analyzers.ANALYSES.items():
            ...
            gList = am['FN_CD_ISSG'](subPageViewSpaceWidth = subPageViewSpaceWidth, 
                                     fn_get_text_pack      = self.visualManager.getTextPack)
            
            #[5-2]: Page TItle Object
            if   am['TYPE'] == 'MAIN': spType = 'MI'
            elif am['TYPE'] == 'SUB':  spType = 'SI'
            else: continue
            ssp.addGUIO("SUBPAGETITLE", generals.passiveGraphics_wrapperTypeC, {'groupOrder': 0, 'xPos':    0, 'yPos': yPos_beg,    'width': subPageViewSpaceWidth, 'height': 300, 'style': 'styleB', 'text': self.visualManager.getTextPack(f'GUIO_CHARTDRAWER:TITLE_{spType}_{amType}'), 'fontSize': 100})
            ssp.addGUIO("NAVBUTTON",    generals.button_typeB,                 {'groupOrder': 0, 'xPos': 3600, 'yPos': yPos_beg+50, 'width':                   400, 'height': 200, 'style': 'styleB', 'image': 'returnIcon_512x512.png', 'imageSize': (170, 170), 'imageRGBA': self.visualManager.getFromColorTable('ICON_COLORING'), 'name': 'navButton_toHome', 'releaseFunction': self.__onSettingsNavButtonClick})
            
            #[5-3]: Line Color
            ssp.addGUIO("INDICATORCOLOR_TITLE",           generals.passiveGraphics_wrapperTypeC, {'groupOrder': 0, 'xPos':    0, 'yPos': yPos_beg-350, 'width': subPageViewSpaceWidth, 'height': 250, 'style': 'styleB', 'text': self.visualManager.getTextPack('GUIO_CHARTDRAWER:LINECOLOR'), 'fontSize': 90, 'anchor': 'SW'})
            ssp.addGUIO("INDICATORCOLOR_TEXT",            generals.textBox_typeA,                {'groupOrder': 0, 'xPos':    0, 'yPos': yPos_beg-700, 'width':  600, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_CHARTDRAWER:LINETARGET'), 'fontSize': 80})
            ssp.addGUIO("INDICATORCOLOR_TARGETSELECTION", generals.selectionBox_typeB,           {'groupOrder': 2, 'xPos':  700, 'yPos': yPos_beg-700, 'width': 1500, 'height': 250, 'style': 'styleA', 'name': f'{amType}_LineSelectionBox', 'nDisplay': 10, 'fontSize': 80, 'selectionUpdateFunction': self.__onSettingsContentUpdate})
            ssp.addGUIO("INDICATORCOLOR_LED",             generals.LED_typeA,                    {'groupOrder': 0, 'xPos': 2300, 'yPos': yPos_beg-700, 'width':  950, 'height': 250, 'style': 'styleA', 'mode': True})
            ssp.addGUIO("INDICATORCOLOR_APPLYCOLOR",      generals.button_typeA,                 {'groupOrder': 0, 'xPos': 3350, 'yPos': yPos_beg-700, 'width':  650, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_CHARTDRAWER:APPLYCOLOR'), 'fontSize': 80, 'name': f'{amType}_ApplyColor', 'releaseFunction': self.__onSettingsContentUpdate})
            for index, componentType in enumerate(('R', 'G', 'B', 'A')):
                ssp.addGUIO(f"INDICATORCOLOR_{componentType}_TEXT",   generals.textBox_typeA, {'groupOrder': 0, 'xPos':    0, 'yPos': yPos_beg-1050-350*index, 'width':  500, 'height': 250, 'style': 'styleA', 'text': componentType, 'fontSize': 80})
                ssp.addGUIO(f"INDICATORCOLOR_{componentType}_SLIDER", generals.slider_typeA,  {'groupOrder': 0, 'xPos':  600, 'yPos': yPos_beg-1050-350*index, 'width': 2600, 'height': 150, 'style': 'styleA', 'name': f'{amType}_Color_{componentType}', 'valueUpdateFunction': self.__onSettingsContentUpdate})
                ssp.addGUIO(f"INDICATORCOLOR_{componentType}_VALUE",  generals.textBox_typeA, {'groupOrder': 0, 'xPos': 3300, 'yPos': yPos_beg-1050-350*index, 'width':  700, 'height': 250, 'style': 'styleA', 'text': "-", 'fontSize': 80})

            #[5-3]: GUIOs Generation Requests Handling
            for gItem in gList:
                #[5-3-1]: Parameters Read
                ...
                gParams['yPos'] = yPos_beg-2100+gParams['yPos']
                if gPOF is not None:
                    gParams[gPOF[0]] = self.__onSettingsContentUpdate
                
                #[5-3-3]: GUIO Generation
                ssp.addGUIO(gName, gType, gParams)

            #[5-4]: Apply New Settings Button
            yPosPoint_min = min(gItem['yPos'] for gItem in gList)
            ssp.addGUIO("APPLYNEWSETTINGS", generals.button_typeA, {'groupOrder': 0, 'xPos': 0, 'yPos': yPos_beg+yPosPoint_min-2450, 'width': subPageViewSpaceWidth, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_CHARTDRAWER:APPLYSETTINGS'), 'fontSize': 80, 'name': f'{amType}_ApplySettings', 'releaseFunction': self.__onSettingsContentUpdate})

            #[5-5]: GUIOs Setup
            am['FN_CD_ISSS'](subpage = ssp, 
                             fn_get_text_pack = self.visualManager.getTextPack)
    ...
    def process(self, t_elapsed_ns):
        ...
        #[3]: Subpage Processing
        self.__process_SubPages(t_elapsed_ns)

        #[4]: Mouse Event Interpretation
        self.__process_MouseEventInterpretation()

        #[5]: PosHighlight Update
        self.__process_PosHighlightUpdate(mei_beg)

        #[6]: Post User Interaction Delay
        ...
        if waitPostDrag or waitPostScroll:
            return

        #[7]: Process Analysis
        if self._process_typeUnique(mei_beg):
            return

        #[8]: Draw Queues Processing
        if self.__process_drawQueues(mei_beg):
            return

        #[9]: RCLCGs Processing
        if self.__process_RCLCGs(mei_beg):
            return

        #[10]: Draw Removal Queues Processing
        self.__process_drawRemovalQueues(mei_beg)
        ...
        #---[3-3]: RELEASED
            ...
            if lastHovered and (lastHovered == 'KLINESPRICE' or lastHovered.startswith('SIVIEWER')):
                ...
                if lastPressed and lastPressed['x'] == ex and lastPressed['y'] == ey:
                    #Left Mouse Button
                    if event['button'] == 1: 
                        self.__updatePosSelection(updateType = 0)
    ...
    def __onPosHighlightUpdate(self):
        ...
        if updated_x:
            ...
            #[3-2]: If Hovering Over A Display Box
                ...
                tMIFound = self.__onPHUs['TRADELOG']()
                if not tMIFound:
                    for amType, am in analyzers.ANALYSES.items():
                        if am['TYPE'] != 'MAIN' or amType not in self.__onPHUs:
                            continue
                        tMIFound = self.__onPHUs[amType](self)
                        if tMIFound: break
    ...
    def on_GUIThemeUpdate(self, **kwargs):
        ...
        abp = self.auxBarPage
        for iID in (KLINE_INTERVAL_ID_1m,
        ...
        for amType, am in analyzers.ANALYSES.items():
            am['FN_CD_OGTU'](subpage              = ssps[amType], 
                             object_configuration = oc,
                             current_GUI_theme    = cgt)
            self.__onSettingsContentUpdate(ssps[amType].GUIOs["INDICATORCOLOR_TARGETSELECTION"])
        ...
        #---Trade Log
            ...
            if (setterType == 'LineSelectionBox'):
                ...
                if updateTracker:
                    ...
                    self.addBufferZone_toDrawQueue(analysisCode = 'TRADELOG', drawSignal     = _FULLDRAWSIGNALS['TRADELOG']) #Update draw queue
                #Control Buttons Handling
                ssps['MAIN'].GUIOs["TRADELOG_APPLYNEWSETTINGS"].deactivate()
                activate_save_config = True
        #---Bids and Asks
        elif indicatorType == 'DEPTHOVERLAY':
            setterType = guioName_split[1]
            if (setterType == 'LineSelectionBox'):
                ...
                if drawSignal:
                    ...
                    self.addBufferZone_toDrawQueue(analysisCode = 'DEPTHOVERLAY', drawSignal     = drawSignal)
                #Control Buttons Handling
        ...
        #[3]: Subpage 'DEPTH'
            ...
            if (setterType == 'LineSelectionBox'):    
                ...
                if drawSignal:
                    ...
                    self.addBufferZone_toDrawQueue(analysisCode = 'DEPTH', drawSignal     = drawSignal)
                #Control Buttons Handling
        ...
        #[4]: Subpage 'AGGTRADE'
            ...
            if (setterType == 'LineSelectionBox'):    
                ...
                if drawSignal:
                    ...
                    self.addBufferZone_toDrawQueue(analysisCode = 'AGGTRADE', drawSignal     = drawSignal)
                #Control Buttons Handling
    ...
    #Drawing --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def addBufferZone_toDrawQueue(self, analysisCode, drawSignal):
        #[1]: Instances
        dRaw   = self._data_raw
    ...
    def __drawer_sendDrawSignals(self, timestamp, analysisCode, redraw = False):
        ...
        try:
            ...
            #[1-2]: Drawing
            isLocal, dFunction = self.__drawerFunctions[analysisCode.split("_")[0]]
            if isLocal: drawn = dFunction(                     drawSignal = drawSignal, timestamp = timestamp, analysisCode = analysisCode)
            else:       drawn = dFunction(chart_drawer = self, drawSignal = drawSignal, timestamp = timestamp, analysisCode = analysisCode)
            if not drawn:
                return

            #[1-3]: Drawn Flags Update
        ...
        #[2]: Exception Handling
            ...
            print(termcolor.colored(f"An unexpected error occured while attempting to draw {analysisCode} at {timestamp}\n", 'light_yellow'), 
        ...
        for aCode in drawn[timestamp]:
            ...
            #[2-8]: INDICATORS
                ...
                am['FN_CD_RMVED'](display_box_graphics = self.displayBox_graphics, 
                                  si_viewer_index      = self.siTypes_siViewerAlloc[targetType] if am['TYPE'] == 'SUB' else None,
                                  analysis_code        = aCode, 
                                  timestamp            = timestamp)

        #[3]: Drawn Flag Removal
        del drawn[timestamp]
    ...
    def _setHVRParams(self):
        self.expectedKlineTemporalWidth = EXPECTEDTEMPORALWIDTHS[self.intervalID]
    ...
    def __onVViewRangeUpdate(self, displayBoxName, updateType):
        ...
        if dBoxName == 'KLINESPRICE': 
            hglCenter = _VVR_CENTERVALUE['KLINESPRICE']
        else:
            siAlloc = oc[f'{dBoxName}SIAlloc']
            if siAlloc in _VVR_CENTERVALUE: hglCenter = _VVR_CENTERVALUE[siAlloc]
            else:                           hglCenter = _VVR_CENTERVALUE[(siAlloc, oc[f'{siAlloc}_DisplayType'])]
    ...
    def _readCurrencyAnalysisConfiguration(self, currencyAnalysisConfiguration):
        ...
        #[2]: Indicators
        for amType, am in analyzers.ANALYSES.items():
            am['FN_CD_LAC'](mainPage               = ssps['MAIN'], 
                            subPage                = ssps[amType],
                            analysis_configuration = cac,
                            object_configuration   = oc)
        ...
        for sivIdx in range (len(_SITYPES)):
            if sivIdx < self.usableSIViewers:
                guios_MAIN[f"SUBINDICATOR_DISPLAYSWITCH{sivIdx}"].activate()
                guios_MAIN[f"SUBINDICATOR_DISPLAYSELECTION{sivIdx}"].activate()
            else:
                guios_MAIN[f"SUBINDICATOR_DISPLAYSWITCH{sivIdx}"].deactivate()
                guios_MAIN[f"SUBINDICATOR_DISPLAYSELECTION{sivIdx}"].deactivate()
    ...
    def _updateSITypeAnalysisCodes(self):
        ...
        #[2]: Analysis Codes Update
        aParams_iID = self.analysisParams.get(self.intervalID)
        for amType, am in analyzers.ANALYSES.items():
            #[2-1]: Analysis Module Type Check
            if am['TYPE'] != 'SUB':
                continue

            #[2-2]: Initialization
            sit_aCodes[amType] = set()

            #[2-3]: Analysis Codes Update
            if aParams_iID is not None:
                aCodes = am['FN_USTAC'](analysis_parameters = aParams_iID)
                sit_aCodes[amType].update(aCodes)
    #Data Control END -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
