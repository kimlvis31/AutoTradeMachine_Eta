...
def __generateAuxillaryFunctions(self):
    ...
    def __displayAssetData():
        ...
        if (_localID == None):
            ...
            else:                                     self.GUIOs["ASSETS_MARGINBALANCEDISPLAYTEXT"].updateText(text = auxiliaries.floatToString(number = _assetData['marginBalance'], precision = _ASSETPRECISIONS[_asset_selected]))
            #Wallet Balance
            if (_assetData['walletBalance'] == None): self.GUIOs["ASSETS_WALLETBALANCEDISPLAYTEXT"].updateText(text = "- / - / -")
            else:
                _wb_total_str    = auxiliaries.floatToString(number = _assetData['walletBalance'],         precision = _ASSETPRECISIONS[_asset_selected])
                _wb_cross_str    = auxiliaries.floatToString(number = _assetData['crossWalletBalance'],    precision = _ASSETPRECISIONS[_asset_selected])
                _wb_isolated_str = auxiliaries.floatToString(number = _assetData['isolatedWalletBalance'], precision = _ASSETPRECISIONS[_asset_selected])
            ...
            else:                                        self.GUIOs["ASSETS_AVAILABLEBALANCEDISPLAYTEXT"].updateText(text = auxiliaries.floatToString(number = _assetData['availableBalance'], precision = _ASSETPRECISIONS[_asset_selected]))
            #Margin
            if (_assetData['crossPositionInitialMargin'] == None): self.GUIOs["ASSETS_MARGINDISPLAYTEXT"].updateText(text = "- / - / -")
            else:
                _mrgn_pi_str    = auxiliaries.floatToString(number = _assetData['crossPositionInitialMargin']+_assetData['isolatedPositionInitialMargin'], precision = _ASSETPRECISIONS[_asset_selected])
                _mrgn_oo_str    = auxiliaries.floatToString(number = _assetData['openOrderInitialMargin'],                                                 precision = _ASSETPRECISIONS[_asset_selected])
                _mrgn_maint_str = auxiliaries.floatToString(number = _assetData['crossMaintenanceMargin'],                                                 precision = _ASSETPRECISIONS[_asset_selected])
            ...
            else:                                          self.GUIOs["ASSETS_ALLOCATABLEBALANCEDISPLAYTEXT"].updateText(text = auxiliaries.floatToString(number = _assetData['allocatableBalance'], precision = _ASSETPRECISIONS[_asset_selected]))
            #Position Initial Margin
            if (_assetData['crossPositionInitialMargin'] == None): self.GUIOs["ASSETS_POSITIONINITIALMARGINDISPLAYTEXT"].updateText(text = "- / -")
            else:
                _piMrgn_cross    = auxiliaries.floatToString(number = _assetData['crossPositionInitialMargin'], precision = _ASSETPRECISIONS[_asset_selected])
                _piMrgn_isolated = auxiliaries.floatToString(number = _assetData['isolatedPositionInitialMargin'], precision = _ASSETPRECISIONS[_asset_selected])
                self.GUIOs["ASSETS_POSITIONINITIALMARGINDISPLAYTEXT"].updateText(text = "{:s} / {:s}".format(_piMrgn_cross, _piMrgn_isolated))
            #Allocated Balance
            self.GUIOs["ASSETS_ALLOCATEDBALANCEDISPLAYTEXT"].updateText(text = auxiliaries.floatToString(number = _assetData['allocatedBalance'], precision = _ASSETPRECISIONS[_asset_selected]))
            #Unrealized PNL
            if (_assetData['unrealizedPNL'] == None): self.GUIOs["ASSETS_UNREALIZEDPNLDISPLAYTEXT"].updateText(text = "- / - / -", textStyle = 'DEFAULT')
                ...
                if (_assetData['unrealizedPNL'] == None): _unPNL_total_str = "-"; _unPNL_total_strColor = 'DEFAULT'
                    ...
                    _unPNL_total_str = auxiliaries.floatToString(number = _assetData['unrealizedPNL'], precision = _ASSETPRECISIONS[_asset_selected])
                ...
                if (_assetData['crossUnrealizedPNL'] == None): _unPNL_cross_str = "-"; _unPNL_cross_strColor = 'DEFAULT'
                    ...
                    _unPNL_cross_str = auxiliaries.floatToString(number = _assetData['crossUnrealizedPNL'], precision = _ASSETPRECISIONS[_asset_selected])
                ...
                if (_assetData['isolatedUnrealizedPNL'] == None): _unPNL_isolated_str = "-"; _unPNL_isolated_strColor = 'DEFAULT'
                    ...
                    _unPNL_isolated_str = auxiliaries.floatToString(number = _assetData['isolatedUnrealizedPNL'], precision = _ASSETPRECISIONS[_asset_selected])
    ...
    def __onPositionSelectionUpdate():
        ...
        if (_displayMode == 'BASIC'):
            ...
            if (_positionSymbol == None):
                ...
                else:                                      self.GUIOs["POSITIONS_TRADERMODESELECTEDMAXALLOCATEDBALANCEVALUETEXTNEW"].updateText(text = auxiliaries.floatToString(number = _maxAllocatedBalance, precision = _position['precisions']['quote']))
