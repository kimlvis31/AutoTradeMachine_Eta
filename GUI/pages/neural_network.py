...
from GUI.generals import passiveGraphics_wrapperTypeC,\
                         ...
                         selectionBox_typeB,\
                         selectionBox_typeC
from GUI.neural_network_viewer import neuralNetworkViewer
...
def __pageEscapeFunction(self):
    for fID in ('onNeuralNetworkUpdate',
                'onProcessUpdate',
                'onCurrenciesUpdate',):
...
def __generateAuxillaryFunctions(self):
            ...
            if (updateType == 'ADDED'):
                ...
                if (_neuralNetworkCode == self.puVar['neuralNetwork_selected']):
                    ...
                    if (_completion == None): 
                        ...
                        else:                           self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_COMPLETIONDISPLAYTEXT"].updateText(text = "{:.3f} % [{:s}]".format(_completion*100, auxiliaries.timeStringFormatter(time_seconds = int(_completion_ETC_s))))
                ...
                if (_processCode == self.puVar['process_selected']):
                    if (_processCode in self.puVar['processes']):
                        ...
                        if (_completion == None):
                            ...
                            else:                           self.GUIOs["NEURALNETWORKMANAGER_PROCESSES_COMPLETIONDISPLAYTEXT"].updateText(text = "{:.3f} % [{:s}]".format(_completion*100, auxiliaries.timeStringFormatter(time_seconds = int(_completion_ETC_s))))
                    else: 
                        self.puVar['process_selected'] = None
                        self.pageAuxillaryFunctions['ONPROCESSSELECTIONUPDATE']()
        ...
        if (self.puVar['neuralNetwork_selected'] == None):
            ...
            if (True):
                ...
                if (_neuralNetwork['completion'] == None):
                    ...
                    else:                                            self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_COMPLETIONDISPLAYTEXT"].updateText(text = "{:.3f} % [{:s}]".format(_neuralNetwork['completion']*100, auxiliaries.timeStringFormatter(time_seconds = int(_neuralNetwork['completion_ETC_s']))))
                self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_GENERATENEURALNETWORKBUTTON"].deactivate()
                self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_INITIALIZATIONBUTTON"].deactivate()
                self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_REMOVENEURALNETWORKBUTTON"].deactivate()
                self.GUIOs["NEURALNETWORKMANAGER_NEURALNETWORKS_CONTROLNEURALNETWORKBUTTONSWITCH"].setStatus(status = False, callStatusUpdateFunction = False)
    ...
    def __setProcessesList():
        ...
        for _pIndex, _pCode in enumerate(self.puVar['processes']):
            ...
            else:                                      _text_ETC = auxiliaries.timeStringFormatter(time_seconds = int(_process['completion_ETC_s']))
            #[7]: Target
    ...
    def __onProcessSelectionUpdate():
        if (self.puVar['process_selected'] == None):
            ...
            if (_process['completion'] == None):
                ...
                else:                                      self.GUIOs["NEURALNETWORKMANAGER_PROCESSES_COMPLETIONDISPLAYTEXT"].updateText(text = "{:.3f} % [{:s}]".format(_process['completion']*100, auxiliaries.timeStringFormatter(time_seconds = int(_process['completion_ETC_s']))))
            self.GUIOs["NEURALNETWORKMANAGER_PROCESSES_REMOVEPROCESSBUTTON"].deactivate()
            self.GUIOs["NEURALNETWORKMANAGER_PROCESSES_REMOVEPROCESSBUTTONSWITCH"].setStatus(status = False, callStatusUpdateFunction = False)
