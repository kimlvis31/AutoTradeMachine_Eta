from GUI import advanced_pyglet_groups, chart_drawer, hit_boxes, text_control
...
class guiPage:
    ...
    def on_GUIThemeUpdate(self):
        ...
        for guioName in self.GUIOs:
            ...
            if ((objectType == text_control.textObject_SL) or (objectType == text_control.textObject_SL_I) or (objectType == text_control.textObject_SL_IE)): self.GUIOs[guioName].on_GUIThemeUpdate(newDefaultTextStyle = self.visualManager.getTextStyle('textInputBox_default')['DEFAULT'])
...
class passiveGraphics_wrapperTypeA:
    def __init__(self, **kwargs):
        ...
        self.textElement = text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.group_1, 
...
class textBox_typeA:
    def __init__(self, **kwargs):
        ...
        #Text Control
        self.textElement = text_control.textObject_SL_I(scaler = self.scaler, batch = self.batch, group = self.group_1, 
...
class button_typeA:
    def __init__(self, **kwargs):
        ...
        for styleMode in self.effectiveTextStyle.keys(): self.effectiveTextStyle[styleMode]['font_size'] = self.fontSize
        self.hitBox = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.width, self.height)
...
class switch_typeB:
    def __init__(self, **kwargs):
        ...
        self.style = kwargs.get('style', 'styleA')
        if (self.align == 'horizontal'): self.hitBox = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.width, self.height)
        elif (self.align == 'vertical'): self.hitBox = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.height, self.width)
...
class slider_typeA:
    def __init__(self, **kwargs):
        ...
        #Hitbox
        if (self.align == 'horizontal'): self.hitBox_slider = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos+self.height/4, self.width, self.height/2)
        elif (self.align == 'vertical'): self.hitBox_slider = hit_boxes.hitBox_Rectangular(self.xPos+self.height/4, self.yPos, self.height/2, self.width)
        self.hitBox_button = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.buttonSize, self.buttonSize)
...
class scrollBar_typeA:
    def __init__(self, **kwargs):
        ...
        if   self.align == 'horizontal': self.hitBox_slider = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.width, self.height)
        elif self.align == 'vertical':   self.hitBox_slider = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.height, self.width)
        self.hitBox_button = hit_boxes.hitBox_Rectangular(0, 0, 0, 0)
...
class textInputBox_typeA:
    def __init__(self, **kwargs):
        ...
        if (self.style == None):
            ...
            self.textElement = text_control.textObject_SL_IE(scaler = self.scaler, batch = self.batch, group = self.group_1, 
                                                                         text = self.visualManager.extractText(self.text), defaultTextStyle = self.effectiveTextStyle['DEFAULT'],
                                                                         xPos = self.xPos, yPos = self.yPos, width = self.width, height = self.height, showElementBox = False, anchor = 'W',
...
class selectionBox_typeA:
    def __init__(self, **kwargs):
        ...
        self.objectHitBox = hit_boxes.hitBox_Rectangular(self.xPos, self.yPos, self.width, self.height)
        ...
        self.displayBox_hitBox = hit_boxes.hitBox_Rectangular(self.displayBox[0], self.displayBox[1], self.displayBox[2], self.displayBox[3])
        ...
        if (groupOrder == None):
            self.display_camGroup = advanced_pyglet_groups.cameraGroup(window = kwargs['windowInstance'], order = self.groupOrder+1, parentCameraGroup = self.group_0,
            ...
            self.display_camGroup = advanced_pyglet_groups.cameraGroup(window = kwargs['windowInstance'], order = self.groupOrder+1,
                                                                                  viewport_x = (self.xPos+self.internalGOffset)*self.scaler, viewport_y = (self.yPos+self.internalGOffset)*self.scaler, viewport_width = self.elementWidth*self.scaler, viewport_height = (self.height-self.internalGOffset*2)*self.scaler,
                                                                                  projection_x0 = 0, projection_x1 = self.elementWidth*self.scaler, projection_y0 = 0, projection_y1 = 1)
            ...
            if (listType == list):
                for index, item in enumerate(selectionList):
                    self.selectionList[index] = {'text': item,
                                                 'textStyles': None,
                                                 'textAnchor': 'CENTER',
                                                 'textElement': text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.display_camGroup, text = "", defaultTextStyle = self.effectiveTextStyle['DEFAULT'], auxillaryTextStyles = self.effectiveTextStyle,
...
class selectionBox_typeC:
    def __init__(self, **kwargs):
        ...
        if groupOrder is None:
            ...
            cg_kwargs = {'order': self.groupOrder+1}
        self.display_camGroup_titles = advanced_pyglet_groups.cameraGroup(window = kwargs['windowInstance'], 
                                                                                   **cg_kwargs,
        ...
        self.display_camGroup_elements = advanced_pyglet_groups.cameraGroup(window = kwargs['windowInstance'], 
                                                                                     **cg_kwargs,
                                                                                     viewport_x      = self.displayBox[0]*self.scaler, 
        ...
        for eWidth in self.elementWidths:
            ct = {'text':       "",
                  'textAnchor': 'CENTER',
                  'textStyles': None,
                  'textElement': text_control.textObject_SL(scaler              = self.scaler, 
                                                                      batch               = self.batch, 
    ...
    def __updateItem(self, itemKey):
        ...
        if item_tes is None and item_display:
            ...
            #---[2-1-2]: Generate new text element instance
                ...
                xCoord = 0
                for cIndex, eWidth in enumerate(self.elementWidths):
                    te = text_control.textObject_SL(scaler              = self.scaler, 
                                                              batch               = self.batch, 
...
class subPageBox_typeA:
    def __init__(self, **kwargs):
        ...
        if (groupOrder == None):
            ...
            self.layeredCamGroup = advanced_pyglet_groups.layeredCameraGroup(window = self.window, parentCameraGroup = self.group_0, viewport_x = spe_viewport_x, viewport_y = spe_viewport_y, viewport_width = spe_viewport_width, viewport_height = spe_viewport_height)
            ...
            self.layeredCamGroup = advanced_pyglet_groups.layeredCameraGroup(window = self.window, order = self.groupOrder, viewport_x = spe_viewport_x, viewport_y = spe_viewport_y, viewport_width = spe_viewport_width, viewport_height = spe_viewport_height)
    ...
    def handleMouseEvent(self, event):
        ...
        if (event['eType'] == "MOVED"): #x, y, dx, dy
            ...
            if (self.selectedGUIO == None):
                ...
                if ((selectedGUIOType == selectionBox_typeA) or (selectedGUIOType == selectionBox_typeB) or (selectedGUIOType == selectionBox_typeC) or (selectedGUIOType == subPageBox_typeA) or (selectedGUIOType == chart_drawer.chartDrawer)): 
