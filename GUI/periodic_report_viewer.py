...
class periodicReportViewer:
            ...
            if (onInit == True):
                for displayBoxName in self.displayBox:
                    ...
                    #---MAIN
                        ...
                        self.displayBox_graphics['MAIN']['DESCRIPTIONTEXT3'] = text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.group_hd0, text = self.visualManager.extractText(self.visualManager.getTextPack(f'GUIO_PERIODICREPORTVIEWER:{self.displayMode}')), 
                                                                                                                    defaultTextStyle = self.effectiveTextStyle['CONTENT_DEFAULT'], auxillaryTextStyles = self.effectiveTextStyle,
    ...
    def __onPosHighlightUpdate(self):
        ...
        if updated_x:
            ...
            #[3-2]: If Hovering Over A Display Box
                ...
                ts_rightEnd = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = tsHovered, mrktReg = None, nTicks = 1)
        ...
        if updated_y:
            ...
            #[3-2]: If Hovering Over A Display Box
                ...
                if formattingMode == 0:
                    ...
                    dBox_g_current['HORIZONTALGUIDETEXT'].setText(text = f"{auxiliaries.simpleValueFormatter(value = yHovered, precision = 3)} %")
                elif formattingMode == 2:
                    dBox_g_current['HORIZONTALGUIDETEXT'].setText(auxiliaries.simpleValueFormatter(value = yHovered, precision = 0))
    ...
    def __onPosHighlightUpdate_updateDataDescriptionText(self):
        ...
        prTS = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = tsHovered, mrktReg = None, nTicks = 0)
    ...
    def __generateDisplayData(self):
        ...
        prs_d = self.periodicReports_display
        func_gnits = auxiliaries.getNextIntervalTickTimestamp
    ...
    def __dataDrawer(self, timestamp):
        ...
        tsWidth = auxiliaries.getNextIntervalTickTimestamp(intervalID = iID_eff, timestamp = timestamp, nTicks = 1)-timestamp
    ...
    def __onHViewRangeUpdate_UpdateGrids(self, updateType):
        ...
        if updateType == 1:
            for giID in auxiliaries.GRID_INTERVAL_IDs[self.intervalID:]:
                rightEnd       = auxiliaries.getNextIntervalTickTimestamp_GRID(giID, hvr_tz_end,           mrktReg = None, nTicks = 1)
                vGridIntervals = auxiliaries.getTimestampList_byRange_GRID(giID,     hvr_tz_beg, rightEnd, mrktReg = None, lastTickInclusive = True)
            ...
            rightEnd       = auxiliaries.getNextIntervalTickTimestamp_GRID(vGridIntervalID_current, hvr_tz_end,           mrktReg = None, nTicks = 1)
            vGridIntervals = auxiliaries.getTimestampList_byRange_GRID(vGridIntervalID_current,     hvr_tz_beg, rightEnd, mrktReg = None, lastTickInclusive = True)
