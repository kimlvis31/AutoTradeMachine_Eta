...
_DATADRAWER_DISPLAYMODE_DATADRAWMETHOD = {'BALANCE':        (0, ('marginBalance_open', 'marginBalance_min', 'marginBalance_max', 'marginBalance_close', 'walletBalance_open', 'walletBalance_min', 'walletBalance_max', 'walletBalance_close')),
                                          ...
                                          'NLIQUIDATIONS':  (2, 'nTrades_liquidation')}

#'hourlyReportViewer' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
class hourlyReportViewer:
    #Initialization -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def __init__(self, **kwargs):
        ...
        self.effectiveTextStyle = self.visualManager.getTextStyle('hourlyReportViewer_'+self.textStyle)
        ...
        if (True):
            self.images['DATALOADINGCOVER'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_dataLoadingCover", self.width*self.scaler, self.height*self.scaler)
        ...
        self.gridColor       = self.visualManager.getFromColorTable('HOURLYREPORTVIEWER_GRID')
        self.gridColor_Heavy = self.visualManager.getFromColorTable('HOURLYREPORTVIEWER_GRIDHEAVY')
        self.guideColor      = self.visualManager.getFromColorTable('HOURLYREPORTVIEWER_GUIDECONTENT')
        self.posHighlightColor_hovered  = self.visualManager.getFromColorTable('HOURLYREPORTVIEWER_POSHOVERED')
        self.posHighlightColor_selected = self.visualManager.getFromColorTable('HOURLYREPORTVIEWER_POSSELECTED')
    ...
    def __configureSettingsSubPageObjects(self):
        ...
        if (True):
            yPos_beg = 20000
            #Title
            self.settingsSubPage.addGUIO("TITLE_MAIN", generals.passiveGraphics_wrapperTypeB, {'groupOrder': 0, 'xPos': 0, 'yPos': yPos_beg, 'width': subPageViewSpaceWidth, 'height': 200, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:TITLE_VIEWERSETTINGS')})
            yPosPoint0 = yPos_beg-350
            self.settingsSubPage.addGUIO("DISPLAYMODE_TEXT",         generals.textBox_typeA,      {'groupOrder': 0, 'xPos':    0, 'yPos':  yPosPoint0,     'width': 1750, 'height': 250, 'style': 'styleB', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:DISPLAYMODE'), 'fontSize': 80})
            self.settingsSubPage.addGUIO("DISPLAYMODE_SELECTIONBOX", generals.selectionBox_typeB, {'groupOrder': 2, 'xPos': 1850, 'yPos':  yPosPoint0,     'width': 2150, 'height': 250, 'style': 'styleA', 'name': 'DISPLAYMODE_SELECTION', 'nDisplay': 10, 'fontSize': 80, 'expansionDir': 0, 'selectionUpdateFunction': self.__onSettingsContentUpdate})
            self.settingsSubPage.addGUIO("TIMEZONE_TEXT",            generals.textBox_typeA,      {'groupOrder': 0, 'xPos':    0, 'yPos':  yPosPoint0-350, 'width': 1750, 'height': 250, 'style': 'styleB', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:TIMEZONE'), 'fontSize': 80})
            self.settingsSubPage.addGUIO("TIMEZONE_SELECTIONBOX",    generals.selectionBox_typeB, {'groupOrder': 2, 'xPos': 1850, 'yPos':  yPosPoint0-350, 'width': 2150, 'height': 250, 'style': 'styleA', 'name': 'TIMEZONE_SELECTION', 'nDisplay': 10, 'fontSize': 80, 'expansionDir': 0, 'selectionUpdateFunction': self.__onSettingsContentUpdate})
            yPosPoint1 = yPosPoint0-650
            self.settingsSubPage.addGUIO("LINECOLOR_TITLE",     generals.passiveGraphics_wrapperTypeC, {'groupOrder': 0, 'xPos':    0, 'yPos': yPosPoint1, 'width': subPageViewSpaceWidth, 'height': 250, 'style': 'styleB', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:LINE'), 'fontSize': 90, 'anchor': 'SW'})
            self.settingsSubPage.addGUIO("LINECOLOR_TEXT",      generals.textBox_typeA,                {'groupOrder': 0, 'xPos':    0, 'yPos': yPosPoint1-350, 'width': 900, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:COLOR'), 'fontSize': 80})
            self.settingsSubPage.addGUIO("LINECOLOR_LED",       generals.LED_typeA,                    {'groupOrder': 0, 'xPos': 1000, 'yPos': yPosPoint1-350, 'width': 950, 'height': 250, 'style': 'styleA', 'mode': True})
            self.settingsSubPage.addGUIO("LINEWIDTH_TEXT",      generals.textBox_typeA,                {'groupOrder': 0, 'xPos': 2050, 'yPos': yPosPoint1-350, 'width': 900, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:WIDTH'), 'fontSize': 80})
            self.settingsSubPage.addGUIO("LINEWIDTH_TEXTINPUT", generals.textInputBox_typeA,           {'groupOrder': 0, 'xPos': 3050, 'yPos': yPosPoint1-350, 'width': 950, 'height': 250, 'style': 'styleA', 'name': 'WidthTextInputBox', 'text': "", 'fontSize': 80, 'textUpdateFunction': self.__onSettingsContentUpdate})
            for index, componentType in enumerate(('R', 'G', 'B', 'A')):
                self.settingsSubPage.addGUIO("LINECOLOR_{:s}_TEXT".format(componentType),   generals.textBox_typeA, {'groupOrder': 0, 'xPos':    0, 'yPos': yPosPoint1-700-350*index, 'width':  500, 'height': 250, 'style': 'styleA', 'text': componentType, 'fontSize': 80})
                self.settingsSubPage.addGUIO("LINECOLOR_{:s}_SLIDER".format(componentType), generals.slider_typeA,  {'groupOrder': 0, 'xPos':  600, 'yPos': yPosPoint1-650-350*index, 'width': 2600, 'height': 150, 'style': 'styleA', 'name': 'Color_{:s}'.format(componentType), 'valueUpdateFunction': self.__onSettingsContentUpdate})
                self.settingsSubPage.addGUIO("LINECOLOR_{:s}_VALUE".format(componentType),  generals.textBox_typeA, {'groupOrder': 0, 'xPos': 3300, 'yPos': yPosPoint1-700-350*index, 'width':  700, 'height': 250, 'style': 'styleA', 'text': "-", 'fontSize': 80})
            yPosPoint2 = yPosPoint1-1750
            self.settingsSubPage.addGUIO("APPLYNEWSETTINGS",  generals.button_typeA, {'groupOrder': 0, 'xPos': 0, 'yPos': yPosPoint2-350, 'width': subPageViewSpaceWidth, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:APPLYSETTINGS'), 'fontSize': 80, 'name': 'ApplySettings',   'releaseFunction': self.__onSettingsContentUpdate})
            self.settingsSubPage.addGUIO("SAVECONFIGURATION", generals.button_typeA, {'groupOrder': 0, 'xPos': 0, 'yPos': yPosPoint2-700, 'width': subPageViewSpaceWidth, 'height': 250, 'style': 'styleA', 'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:SAVECONFIG'),    'fontSize': 80, 'name': 'SAVECONFIG', 'releaseFunction': self.__onSettingsContentUpdate})
            #GUIO Setup
            displayModeSelections = {'BALANCE':        {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:BALANCE')},
                                     'COMMITMENTRATE': {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:COMMITMENTRATE')},
                                     'RISKLEVEL':      {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:RISKLEVEL')},
                                     'NTRADES_TOTAL':  {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:NTRADES_TOTAL')},
                                     'NTRADES_BUY':    {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:NTRADES_BUY')},
                                     'NTRADES_SELL':   {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:NTRADES_SELL')},
                                     'NTRADES_PSL':    {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:NTRADES_PSL')},
                                     'NLIQUIDATIONS':  {'text': self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:NLIQUIDATIONS')}}
            ...
            if (onInit == True):
                for displayBoxName in self.displayBox:
                    ...
                    if (displayBoxName == 'MAINGRID_X'):
                        ...
                        self.hitBox['MAINGRID_X'] = hit_boxes.hitBox_Rectangular(drawBox[0], drawBox[1], drawBox[2], drawBox[3])
                        self.images['MAINGRID_X'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrame", displayBox[2]*self.scaler, displayBox[3]*self.scaler)
                    ...
                    #---SETTINGSBUTTONFRAME
                        ...
                        self.images['SETTINGSBUTTONFRAME_DEFAULT'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrameInteractable_DEFAULT", self.displayBox['SETTINGSBUTTONFRAME'][2]*self.scaler, self.displayBox['SETTINGSBUTTONFRAME'][3]*self.scaler)
                        self.images['SETTINGSBUTTONFRAME_HOVERED'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrameInteractable_HOVERED", self.displayBox['SETTINGSBUTTONFRAME'][2]*self.scaler, self.displayBox['SETTINGSBUTTONFRAME'][3]*self.scaler)
                        self.images['SETTINGSBUTTONFRAME_PRESSED'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrameInteractable_PRESSED", self.displayBox['SETTINGSBUTTONFRAME'][2]*self.scaler, self.displayBox['SETTINGSBUTTONFRAME'][3]*self.scaler)
                    ...
                    #---MAIN
                        ...
                        #Generate Graphic Sprites and Hitboxes
                        self.hitBox['MAIN'] = hit_boxes.hitBox_Rectangular(drawBox[0], drawBox[1], drawBox[2], drawBox[3])
                        self.images['MAIN'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrame", displayBox[2]*self.scaler, displayBox[3]*self.scaler)
                        ...
                        self.hitBox['MAINGRID_Y'] = hit_boxes.hitBox_Rectangular(drawBox_MAINGRID_Y[0], drawBox_MAINGRID_Y[1], drawBox_MAINGRID_Y[2], drawBox_MAINGRID_Y[3])
                        self.images['MAINGRID_Y'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrame", displayBox_MAINGRID_Y[2]*self.scaler, displayBox_MAINGRID_Y[3]*self.scaler)
                        ...
                        #Description Texts
                        self.displayBox_graphics['MAIN']['DESCRIPTIONTEXT1'] = text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.group_hd0, text = "", 
                        ...
                        self.displayBox_graphics['MAIN']['DESCRIPTIONTEXT2'] = text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.group_hd0, text = "", 
                                                                                                                    defaultTextStyle = self.effectiveTextStyle['CONTENT_DEFAULT'], auxillaryTextStyles = self.effectiveTextStyle,
                                                                                                                    xPos = drawBox[0], yPos = drawBox[1]+drawBox[3]-400, width = drawBox[2], height = 200, showElementBox = False, anchor = 'W')
                        self.displayBox_graphics['MAIN']['DESCRIPTIONTEXT3'] = text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.group_hd0, text = "", 
                                                                                                                    defaultTextStyle = self.effectiveTextStyle['CONTENT_DEFAULT'], auxillaryTextStyles = self.effectiveTextStyle,
                        ...
                        for i in range (self.nMaxHorizontalGridLines):
                            ...
                            self.displayBox_graphics['MAINGRID_Y']['HORIZONTALGRID_TEXTS'].append(text_control.textObject_SL(scaler = self.scaler, batch = self.batch, group = self.displayBox_graphics['MAINGRID_Y']['HORIZONTALGRID_CAMGROUP'], text = "-", defaultTextStyle = self.effectiveTextStyle['GRID'],
                ...
                for displayBoxName in self.displayBox:
                    if (displayBoxName in self.displayBox_VisibleBoxes):
                        ...
                        #MAIN
                            ...
                            self.images['MAINGRID_Y'] = self.imageManager.getImageByCode("hourlyReportViewer_typeA_"+self.style+"_displayBoxFrame", drawBox_MAINGRID_Y[2]*self.scaler, drawBox_MAINGRID_Y[3]*self.scaler)
    ...
    def __initializeRCLCG(self):
        ...
        elif (self.data_DisplayMode == 'NTRADES_BUY'):    self.verticalViewRange_precision = 0
        elif (self.data_DisplayMode == 'NTRADES_SELL'):   self.verticalViewRange_precision = 0
        elif (self.data_DisplayMode == 'NTRADES_PSL'):    self.verticalViewRange_precision = 0
        elif (self.data_DisplayMode == 'NLIQUIDATIONS'):  self.verticalViewRange_precision = 0
        precision_x = math.floor(math.log(self.expectedKlineTemporalWidth, 10))
        precision_y = -self.verticalViewRange_precision
        if ('RCLCG' in self.displayBox_graphics['MAIN']):
    ...
    def __updatePosHighlight(self, mouseX, mouseY, hoveredSection, updateType):
        if (updateType == 0):
            try:
                ...
                tsIntervalHovered_0 = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = xValHovered, mrktReg = None, nTicks = 0)
    ...
    def __onPosHighlightUpdate(self):
        ...
        if (self.posHighlight_updatedPositions[0] == True):
            if (self.posHighlight_hoveredPos[2] == None): 
                ...
                ts_rightEnd = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = self.posHighlight_hoveredPos[0], mrktReg = None, nTicks = 1)
        ...
        if (self.posHighlight_updatedPositions[1] == True):
            if (self.posHighlight_hoveredPos[2] == None):
                ...
                self.displayBox_graphics[dBox_current]['HORIZONTALGUIDETEXT'].setText(auxiliaries.floatToString(number = self.posHighlight_hoveredPos[1], precision = self.verticalViewRange_precision))
        self.posHighlight_updatedPositions = None
    ...
    def __updatePosSelection(self, updateType):
        ...
        #By HorizontalViewRange Update
            ...
            if (self.posHighlight_selectedPos != None):
                tsPosEnd = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = self.posHighlight_selectedPos, mrktReg = None, nTicks = 1)
                pixelPerTS = self.displayBox_graphics['MAINGRID_X']['DRAWBOX'][2]*self.scaler / (self.horizontalViewRange[1]-self.horizontalViewRange[0])
    ...
    def on_GUIThemeUpdate(self, **kwargs):
        ...
        newEffectiveTextStyle = self.visualManager.getTextStyle('hourlyReportViewer_'+self.textStyle)
    ...
    def __onSettingsContentUpdate(self, objectInstnace):
        ...
        if (setterType == 'DISPLAYMODE'):
            ...
            if ('DESCRIPTIONTEXT3' in self.displayBox_graphics['MAIN']): self.displayBox_graphics['MAIN']['DESCRIPTIONTEXT3'].setText(self.visualManager.extractText(self.visualManager.getTextPack('GUIO_HOURLYREPORTVIEWER:{:s}'.format(selectedDisplayMode))))
            try:
    ...
    def updateTimeZone(self, newTimeZone):
        ...
        for index in range (len(self.verticalGrid_intervals)):
            ...
            if (self.verticalGrid_intervalID <= 10):
                ...
                if (auxiliaries.isNewMonth(timestamp_display) == True): dateStrFormat = "%Y/%m"
                else:                                                          dateStrFormat = "%m/%d"
    ...
    def __dataDrawer(self, timestamp):
        _hourlyReport = self.data_ForDisplay[timestamp]
        _tsWidth = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = timestamp, mrktReg = None, nTicks = 1)-timestamp
        ...
        if (_drawType == 0):
            ...
            #Y1
            _d0 = _hourlyReport[_dataCall[0]]
            _d1 = _hourlyReport[_dataCall[1]]
            _d2 = _hourlyReport[_dataCall[2]]
            _d3 = _hourlyReport[_dataCall[3]]
            if (_d0 < _d3): _body_y1 = _d0; _body_height1 = _d3-_d0
            else:           _body_y1 = _d3; _body_height1 = _d0-_d3
            _tail_y1      = _d1
            _tail_height1 = _d2-_d1
            #Y2
            _d4 = _hourlyReport[_dataCall[4]]
            _d5 = _hourlyReport[_dataCall[5]]
            _d6 = _hourlyReport[_dataCall[6]]
            _d7 = _hourlyReport[_dataCall[7]]
        ...
        #---DrawType 1
            ...
            _tail_xPos  = round(timestamp+(_tsWidth-_tail_width)/2, 1)
            #Y
            _d0 = _hourlyReport[_dataCall[0]]*100
            _d1 = _hourlyReport[_dataCall[1]]*100
            _d2 = _hourlyReport[_dataCall[2]]*100
            _d3 = _hourlyReport[_dataCall[3]]*100
        ...
        #---DrawType 2
            ...
            #Drawing
            self.displayBox_graphics['MAIN']['RCLCG'].addShape_Rectangle(x = timestamp, width = _tsWidth, y = 0, height = _hourlyReport[_dataCall], color = color, shapeName = timestamp, shapeGroupName = '0', layerNumber = 1)
        return True
    ...
    def __onHViewRangeUpdate(self, updateType):
        ...
        if (self.__checkVerticalExtremas() == True): self.__onVerticalExtremaUpdate(updateType = updateType)
    ...
    def _onHViewRangeUpdate_UpdateProcessQueue(self):
        ...
        self.horizontalViewRange_timestampsInViewRange = set(auxiliaries.getTimestampList_byRange(self.intervalID, self.horizontalViewRange[0], self.horizontalViewRange[1], lastTickInclusive = True))
        nTSsInViewRange = len(self.horizontalViewRange_timestampsInViewRange)
        timestampsInBufferZone1 = set(auxiliaries.getTimestampList_byNTicks(self.intervalID, self.horizontalViewRange[0], nTicks = int(nTSsInViewRange*_GD_DISPLAYBOX_HVR_BACKWARDBUFFERFACTOR)+1, direction = False, mrktReg = None)[1:])
        timestampsInBufferZone2 = set(auxiliaries.getTimestampList_byNTicks(self.intervalID, self.horizontalViewRange[1], nTicks = int(nTSsInViewRange*_GD_DISPLAYBOX_HVR_FORWARDBUFFERFACTOR) +1, direction = True,  mrktReg = None)[1:])
    ...
    def __onHViewRangeUpdate_UpdateGrids(self, updateType):
        ...
        if (updateType == 1):
            for gridIntervalID in auxiliaries.GRID_INTERVAL_IDs[self.intervalID:]:
                rightEnd = auxiliaries.getNextIntervalTickTimestamp_GRID(gridIntervalID, self.horizontalViewRange[1], mrktReg = None, nTicks = 1)
                verticalGrid_intervals = auxiliaries.getTimestampList_byRange_GRID(gridIntervalID, self.horizontalViewRange[0], rightEnd, mrktReg = None, lastTickInclusive = True)
            ...
            rightEnd = auxiliaries.getNextIntervalTickTimestamp_GRID(self.verticalGrid_intervalID, self.horizontalViewRange[1], mrktReg = None, nTicks = 1)
            verticalGrid_intervals = auxiliaries.getTimestampList_byRange_GRID(self.verticalGrid_intervalID, self.horizontalViewRange[0], rightEnd, mrktReg = None, lastTickInclusive = True)
    ...
    def __checkVerticalExtremas(self):
        ...
        for _ts in self.horizontalViewRange_timestampsInViewRange:
            if (_ts in self.data_ForDisplay):
                #Values Collection
                _hourlyReport = self.data_ForDisplay[_ts]
                if   (self.data_DisplayMode == 'BALANCE'):        values = [_hourlyReport['marginBalance_min'], _hourlyReport['marginBalance_max'], _hourlyReport['walletBalance_min'], _hourlyReport['walletBalance_max']]
                elif (self.data_DisplayMode == 'COMMITMENTRATE'): values = [_hourlyReport['commitmentRate_min']*100, _hourlyReport['commitmentRate_max']*100]
                elif (self.data_DisplayMode == 'RISKLEVEL'):      values = [_hourlyReport['riskLevel_min']*100, _hourlyReport['riskLevel_max']*100]
                elif (self.data_DisplayMode == 'NTRADES_TOTAL'):  values = [_hourlyReport['nTrades'],]
                elif (self.data_DisplayMode == 'NTRADES_BUY'):    values = [_hourlyReport['nTrades_buy'],]
                elif (self.data_DisplayMode == 'NTRADES_SELL'):   values = [_hourlyReport['nTrades_sell'],]
                elif (self.data_DisplayMode == 'NTRADES_PSL'):    values = [_hourlyReport['nTrades_psl'],]
                elif (self.data_DisplayMode == 'NLIQUIDATIONS'):  values = [_hourlyReport['nTrades_liquidation'],]
                #Extrema Finding
                if (0 < len(values)):
        ...
        if (((valMin != float('inf')) and (valMax != float('-inf'))) and ((self.verticalValue_min != valMin) or (self.verticalValue_max != valMax))): #The found extremas are different
            if (round(valMin, self.verticalViewRange_precision) == round(valMax, self.verticalViewRange_precision)):
        ...
        if (gridContentsUpdateFlag == True):
            ...
            for index in range (self.nMaxHorizontalGridLines):
                if (index < len(self.horizontalGridIntervals)):
                    ...
                    else:                    verticalValue_formatted = auxiliaries.simpleValueFormatter(value = verticalValue, precision = 2)
            ...
            if ((_localID == self.localID) and (requestID == self.data_Fetching_RID)):
                if (_result == True):
                    ...
                    for _ts in self.data_ForDisplay:
                        t_open  = _ts
                        t_close = auxiliaries.getNextIntervalTickTimestamp(intervalID = self.intervalID, timestamp = t_open, mrktReg = None, nTicks = 1)-1
                        classification = 0
