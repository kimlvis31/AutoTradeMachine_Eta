...
def cd_initialize_settings_subpage_generate(subPageViewSpaceWidth, fn_get_text_pack):
    ...
    for lIdx in range (NMAXLINES):
        ...
        gList.append(({'NAME':               f"INDICATOR_DMIxADX{lIdx}_INTERVALINPUT",
                       ...
                       'groupOrder': 0, 'xPos': 1100, 'yPos': yPosPoint1-350*(lIdx+1), 'width':  900, 'height': 250, 'style': 'styleA', 'text': "", 'fontSize': 80, 'name': f'DMIxADX_IntervalTextInputBox_{lIdx}'}))
        gList.append(({'NAME':               f"INDICATOR_DMIxADX{lIdx}_WIDTHINPUT",
                       'TYPE':               'textInputBox_typeA',
                       'PAGEOBJECTFUNCTION': ['textUpdateFunction',],
                       'groupOrder': 0, 'xPos': 2100, 'yPos': yPosPoint1-350*(lIdx+1), 'width':  600, 'height': 250, 'style': 'styleA', 'text': "", 'fontSize': 80, 'name': f'DMIxADX_WidthTextInputBox_{lIdx}'}))
