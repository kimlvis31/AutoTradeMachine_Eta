...
def get_maximum_market_data_reference_length(cac_iID):
    ...
    for lIdx in range (NMAXLINES):
        ...
        if not cac_iID.get(f'EMA_{lIdx}_LineActive', False): 
            continue

        #[2-2]: MMDRL Update
        mmdrl = max(mmdrl, 
                    cac_iID[f'EMA_{lIdx}_NSamples'])
...
def cd_get_initial_configuration():
    ...
    oc['EMA_Master'] = False
    for lIdx in range (NMAXLINES):
        oc[f'EMA_{lIdx}_LineActive'] = False
        oc[f'EMA_{lIdx}_NSamples']   = 10*(lIdx+1)
        oc[f'EMA_{lIdx}_Width'] = 1
        oc[f'EMA_{lIdx}_ColorR%DARK'] =random.randint(64,255); oc[f'EMA_{lIdx}_ColorG%DARK'] =random.randint(64,255); oc[f'EMA_{lIdx}_ColorB%DARK'] =random.randint(64, 255); oc[f'EMA_{lIdx}_ColorA%DARK'] =255
        oc[f'EMA_{lIdx}_ColorR%LIGHT']=random.randint(64,255); oc[f'EMA_{lIdx}_ColorG%LIGHT']=random.randint(64,255); oc[f'EMA_{lIdx}_ColorB%LIGHT']=random.randint(64, 255); oc[f'EMA_{lIdx}_ColorA%LIGHT']=255
        oc[f'EMA_{lIdx}_Display'] = True
...
def cd_initialize_settings_subpage_generate(subPageViewSpaceWidth, fn_get_text_pack):
    ...
    yPosPoint1 = -300
    for lIdx in range (NMAXLINES):
        gList.append(({'NAME':               f"INDICATOR_EMA{lIdx}",
                       'TYPE':               'switch_typeC',
                       'PAGEOBJECTFUNCTION': ['statusUpdateFunction',],
                       'groupOrder': 0, 'xPos':    0, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 800, 'height': 250, 'style': 'styleB', 'text': f'EMA {lIdx}', 'fontSize': 80, 'name': f'EMA_LineActivationSwitch_{lIdx}'}))
        gList.append(({'NAME':               f"INDICATOR_EMA{lIdx}_INTERVALINPUT",
                       'TYPE':               'textInputBox_typeA',
                       'PAGEOBJECTFUNCTION': ['textUpdateFunction',],
                       'groupOrder': 0, 'xPos':  900, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 900, 'height': 250, 'style': 'styleA', 'text': "", 'fontSize': 80, 'name': f'EMA_IntervalTextInputBox_{lIdx}'}))
        gList.append(({'NAME':               f"INDICATOR_EMA{lIdx}_WIDTHINPUT",
                       'TYPE':               'textInputBox_typeA',
                       'PAGEOBJECTFUNCTION': ['textUpdateFunction',],
                       'groupOrder': 0, 'xPos': 1900, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 750, 'height': 250, 'style': 'styleA', 'text': "", 'fontSize': 80, 'name': f'EMA_WidthTextInputBox_{lIdx}'}))
        gList.append(({'NAME':               f"INDICATOR_EMA{lIdx}_LINECOLOR",
                       'TYPE':               'LED_typeA',
                       'PAGEOBJECTFUNCTION': None,
                       'groupOrder': 0, 'xPos': 2750, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 650, 'height': 250, 'style': 'styleA', 'fontSize': 80, 'mode': True}))
        gList.append(({'NAME':               f"INDICATOR_EMA{lIdx}_DISPLAY",
                       'TYPE':               'switch_typeB',
                       'PAGEOBJECTFUNCTION': ['releaseFunction',],
                       'groupOrder': 0, 'xPos': 3500, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 500, 'height': 250, 'style': 'styleA', 'fontSize': 80, 'name': f'EMA_DisplaySwitch_{lIdx}'}))
...
def cd_initialize_settings_subpage_setup(subpage, fn_get_text_pack):
    subpage.GUIOs["INDICATORCOLOR_TARGETSELECTION"].setSelectionList(selectionList  = {f"{lIdx}": {'text': f"EMA {lIdx}"} for lIdx in range (NMAXLINES)}, 
                                                                     displayTargets = 'all')
...
def cd_match_guios_to_config(mainPage, subPage, current_GUI_Theme, object_configuration):
    ...
    for lIdx in range (NMAXLINES):
        lActive  = oc[f'EMA_{lIdx}_LineActive']
        nSamples = oc[f'EMA_{lIdx}_NSamples']
        width    = oc[f'EMA_{lIdx}_Width']
        color    = (oc[f'EMA_{lIdx}_ColorR%{cgt}'],
                    oc[f'EMA_{lIdx}_ColorG%{cgt}'],
                    oc[f'EMA_{lIdx}_ColorB%{cgt}'],
                    oc[f'EMA_{lIdx}_ColorA%{cgt}'])
        display  = oc[f'EMA_{lIdx}_Display']
        guios_THIS[f"INDICATOR_EMA{lIdx}"].setStatus(status = lActive, callStatusUpdateFunction = False)
        guios_THIS[f"INDICATOR_EMA{lIdx}_INTERVALINPUT"].updateText(text = f"{nSamples}")
        guios_THIS[f"INDICATOR_EMA{lIdx}_WIDTHINPUT"].updateText(text = f"{width}")
        guios_THIS[f"INDICATOR_EMA{lIdx}_LINECOLOR"].updateColor(*color)
        guios_THIS[f"INDICATOR_EMA{lIdx}_DISPLAY"].setStatus(status = display, callStatusUpdateFunction = False)
...
def cd_load_analysis_configuration(mainPage, subPage, analysis_configuration, object_configuration):
    ...
    #[2]: GUIOs Update
    if ac is not None and ac['EMA_Master']:
        ...
        for lIdx in range (NMAXLINES):
            if ac[f'EMA_{lIdx}_LineActive']:
                nSamples = ac[f'EMA_{lIdx}_NSamples']
                width    = oc[f'EMA_{lIdx}_Width']
                display  = oc[f'EMA_{lIdx}_Display']
                guios_THIS[f"INDICATOR_EMA{lIdx}"].setStatus(status = True, callStatusUpdateFunction = False)
                guios_THIS[f"INDICATOR_EMA{lIdx}_INTERVALINPUT"].updateText(f"{nSamples}")
                guios_THIS[f"INDICATOR_EMA{lIdx}_WIDTHINPUT"].activate()
                guios_THIS[f"INDICATOR_EMA{lIdx}_WIDTHINPUT"].updateText(f"{width}")
                guios_THIS[f"INDICATOR_EMA{lIdx}_DISPLAY"].setStatus(status = display, callStatusUpdateFunction = False)
                guios_THIS[f"INDICATOR_EMA{lIdx}_DISPLAY"].activate()
            else:
                guios_THIS[f"INDICATOR_EMA{lIdx}"].setStatus(status = False, callStatusUpdateFunction = False)
                guios_THIS[f"INDICATOR_EMA{lIdx}_INTERVALINPUT"].updateText("-")
                guios_THIS[f"INDICATOR_EMA{lIdx}_WIDTHINPUT"].deactivate()
                guios_THIS[f"INDICATOR_EMA{lIdx}_DISPLAY"].deactivate()
                guios_THIS[f"INDICATOR_EMA{lIdx}_DISPLAY"].setStatus(status = False, callStatusUpdateFunction = False)
    else:
...
def cd_on_settings_content_update(chart_drawer, main_page, sub_page, guio_name_split):
    ...
    if setter == 'LineSelectionBox':
        ...
        color_r, color_g, color_b, color_a = sub_page.GUIOs[f"INDICATOR_EMA{lineSelected}_LINECOLOR"].getColor()
    ...
    #---[2-3]: ApplyColor
        ...
        sub_page.GUIOs[f"INDICATOR_EMA{lineSelected}_LINECOLOR"].updateColor(color_r, color_g, color_b, color_a)
        sub_page.GUIOs['INDICATORCOLOR_APPLYCOLOR'].deactivate()
    ...
    #---[2-6]: ApplySettings
        ...
        for lineIndex in range (NMAXLINES):
            ...
            #Width
            width_previous = oc[f'EMA_{lineIndex}_Width']
            reset = False
            try:
                width = int(sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}_WIDTHINPUT"].getText())
                if 0 < width: oc[f'EMA_{lineIndex}_Width'] = width
                else: reset = True
            except: reset = True
            if reset:
                oc[f'EMA_{lineIndex}_Width'] = 1
                sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}_WIDTHINPUT"].updateText(str(oc[f'EMA_{lineIndex}_Width']))
            if width_previous != oc[f'EMA_{lineIndex}_Width']: updateTracker[lineIndex] = True
            #Color
            color_previous = (oc[f'EMA_{lineIndex}_ColorR%{cgt}'], 
                              oc[f'EMA_{lineIndex}_ColorG%{cgt}'], 
                              oc[f'EMA_{lineIndex}_ColorB%{cgt}'], 
                              oc[f'EMA_{lineIndex}_ColorA%{cgt}'])
            color_r, color_g, color_b, color_a = sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}_LINECOLOR"].getColor()
            oc[f'EMA_{lineIndex}_ColorR%{cgt}'] = color_r
            oc[f'EMA_{lineIndex}_ColorG%{cgt}'] = color_g
            oc[f'EMA_{lineIndex}_ColorB%{cgt}'] = color_b
            oc[f'EMA_{lineIndex}_ColorA%{cgt}'] = color_a
            if (color_previous != (color_r, color_g, color_b, color_a)): updateTracker[lineIndex] = True
            #Line Display
            display_previous = oc[f'EMA_{lineIndex}_Display']
            oc[f'EMA_{lineIndex}_Display'] = sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}_DISPLAY"].getStatus()
            if display_previous != oc[f'EMA_{lineIndex}_Display']: updateTracker[lineIndex] = True
        #MA Master
        maMaster_previous = oc[f'EMA_Master']
        oc[f'EMA_Master'] = main_page.GUIOs[f"MAININDICATOR_EMA"].getStatus()
        if maMaster_previous != oc[f'EMA_Master']:
            for lineIndex in updateTracker: updateTracker[lineIndex] = True
        #Queue Update
        for line in [aCode for aCode in analysis_parameters if aCode.startswith('EMA')]:
    ...
    #---[3-1]: Line Activation Switch
        ...
        _newStatus = sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}"].getStatus()
        oc[f'EMA_{lineIndex}_LineActive'] = _newStatus
    ...
    #---[3-2]: Interval Text Input Box
        ...
        lineIndex = int(guio_name_split[2])
        #Get new nSamples
        try:    _nSamples = int(sub_page.GUIOs[f"INDICATOR_EMA{lineIndex}_INTERVALINPUT"].getText())
        except: _nSamples = None
        #Save the new value to the object config dictionary
        oc[f'EMA_{lineIndex}_NSamples'] = _nSamples
        #Analysis Configuration Update Response
        chart_drawer._onAnalysisConfigurationUpdate()
        activate_save_configuration = True
...
def cd_draw(chart_drawer, drawSignal, timestamp, analysisCode):
    ...
    #[4]: Data Acquisition
    emas = chart_drawer._data_agg[chart_drawer.intervalID][analysisCode]
...
def cd_on_GUI_theme_update(subpage, object_configuration, current_GUI_theme):
    ...
    #[2]: GUIOs Update
    for lIdx in range (NMAXLINES):
        sp.GUIOs[f"INDICATOR_EMA{lIdx}_LINECOLOR"].updateColor(oc[f'EMA_{lIdx}_ColorR%{cgt}'], 
                                                               oc[f'EMA_{lIdx}_ColorG%{cgt}'], 
                                                               oc[f'EMA_{lIdx}_ColorB%{cgt}'], 
                                                               oc[f'EMA_{lIdx}_ColorA%{cgt}'])
...
def cd_type_init(subPage):
    ...
    #[2]: GUIOs Update
    for lIdx in range (NMAXLINES):
        guios_THIS[f"INDICATOR_EMA{lIdx}"].deactivate()
        guios_THIS[f"INDICATOR_EMA{lIdx}_INTERVALINPUT"].deactivate()
...
def pg_autotrade_get_default_analysis_configuration():
    ...
    #[2]: Setup
    dac['EMA_Master'] = False
    for lIdx in range (NMAXLINES):
        dac[f'EMA_{lIdx}_LineActive'] = False
        dac[f'EMA_{lIdx}_NSamples']   = 10*(lIdx+1)
...
def pg_autotrade_configure_subpage_generate(subPageViewSpaceWidth, fn_get_text_pack):
    ...
    yPosPoint1 = -300
    for lIdx in range (NMAXLINES):
        gList.append(({'NAME':               f"EMA_{lIdx}_LINE",
                       'TYPE':               'switch_typeC',
                       'PAGEOBJECTFUNCTION': None,
                       'groupOrder': 0, 'xPos':    0, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 2225, 'height': 250, 'style': 'styleB', 'text': f'EMA {lIdx}', 'fontSize': 80}))
        gList.append(({'NAME':               f"EMA_{lIdx}_NSAMPLES",
                       'TYPE':               'textInputBox_typeA',
                       'PAGEOBJECTFUNCTION': None,
...
def pg_autotrade_load_analysis_configuration(mainPage, subPage, analysis_configuration):
    #[1]: Main Page
    mainPage.GUIOs["INDICATORMASTERSWITCH_EMA"].setStatus(status = analysis_configuration['EMA_Master'], callStatusUpdateFunction = False)

    #[2]: Sub Page
    for lIdx in range (NMAXLINES):
        #[2-1]: Configuration Retrieval
        if f'EMA_{lIdx}_LineActive' in analysis_configuration:
            lineActive = analysis_configuration[f'EMA_{lIdx}_LineActive']
            nSamples   = analysis_configuration[f'EMA_{lIdx}_NSamples']
        else:
            lineActive = False
            nSamples   = 10*(lIdx+1)

        #[2-2]: GUIOs Update
        subPage.GUIOs[f"EMA_{lIdx}_LINE"].setStatus(status = lineActive, callStatusUpdateFunction = False)
        subPage.GUIOs[f"EMA_{lIdx}_NSAMPLES"].updateText(text = f"{nSamples}")
...
def pg_autotrade_format_analysis_configuration_from_guios(mainPage, subPage):
    ...
    configuration['EMA_Master'] = mainPage.GUIOs["INDICATORMASTERSWITCH_EMA"].getStatus()
    for lineIndex in range (NMAXLINES):
        configuration[f'EMA_{lineIndex}_LineActive'] = subPage.GUIOs[f"EMA_{lineIndex}_LINE"].getStatus()
        configuration[f'EMA_{lineIndex}_NSamples']   = int(subPage.GUIOs[f"EMA_{lineIndex}_NSAMPLES"].getText())
...
def pg_simulation_result_configure_subpage_generate(subPageViewSpaceWidth, fn_get_text_pack):
    ...
    for lIdx in range (NMAXLINES):
        gList.append(({'NAME':               f"EMA_{lIdx}_LINE",
                       'TYPE':               'switch_typeC',
                       'PAGEOBJECTFUNCTION': None,
                       'groupOrder': 0, 'xPos': 0, 'yPos': yPosPoint1-350*(lIdx+1), 'width': 2525, 'height': 250, 'style': 'styleB', 'text': f'EMA {lIdx}', 'fontSize': 80}))
        gList.append(({'NAME':               f"EMA_{lIdx}_NSAMPLES",
                       'TYPE':               'textBox_typeA',
                       'PAGEOBJECTFUNCTION': None,
...
def pg_simulation_result_configure_subpage_setup(subpage, fn_get_text_pack):
    for lIdx in range (NMAXLINES):
        subpage.GUIOs[f"EMA_{lIdx}_LINE"].deactivate()
...
def pg_simulation_result_load_analysis_configuration(mainPage, subPage, analysis_configuration, simulation_selected, fn_get_text_pack):
    if simulation_selected:
        mainPage.GUIOs["INDICATORMASTERSWITCH_EMA"].setStatus(status = analysis_configuration['EMA_Master'], callStatusUpdateFunction = False)
        for lIdx in range (NMAXLINES):
            lineActive = analysis_configuration.get(f'EMA_{lIdx}_LineActive', False)
            if lineActive: nSamples_str = f"{analysis_configuration[f'EMA_{lIdx}_NSamples']}"
            else:          nSamples_str = "-"
            subPage.GUIOs[f"EMA_{lIdx}_LINE"].setStatus(status = lineActive, callStatusUpdateFunction = False)
            subPage.GUIOs[f"EMA_{lIdx}_NSAMPLES"].updateText(text = nSamples_str)
    else:
        mainPage.GUIOs["INDICATORMASTERSWITCH_EMA"].setStatus(status = False, callStatusUpdateFunction = False)
        for lIdx in range (NMAXLINES):
            subPage.GUIOs[f"EMA_{lIdx}_LINE"].setStatus(status = False, callStatusUpdateFunction = False)
            subPage.GUIOs[f"EMA_{lIdx}_NSAMPLES"].updateText(text = "-")
