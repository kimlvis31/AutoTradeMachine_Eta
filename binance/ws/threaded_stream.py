...
class ThreadedApiManager(threading.Thread):
    ...
    async def start_listener(self, socket, path: str, callback):
        async with socket as s:
            while ((path in self._socket_running) and (self._socket_running[path] == True)):
                try:
                ...
                if asyncio.iscoroutinefunction(callback): asyncio.create_task(callback(msg))
                else:                                     callback(msg)
        if (path in self._socket_running): del self._socket_running[path]
