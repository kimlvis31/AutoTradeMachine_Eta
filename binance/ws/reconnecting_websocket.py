...
class ReconnectingWebsocket:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self._handle_read_loop:                         await self._kill_read_loop()
        if self._exit_coro:                                await self._exit_coro(self._path)
        if self.ws:                                        await self.ws.close()
        if self._conn and hasattr(self._conn, "protocol"): await self._conn.__aexit__(exc_type, exc_val, exc_tb)
        self.ws = None
    ...
    async def _read_loop(self):
        try:
            while True:
                try:
                    while self.ws_state == WSListenerState.RECONNECTING: await self._run_reconnect()
                    ...
                    if (self.ws is None):
                        ...
                        if self.ws_state == WSListenerState.EXITING:
                            ...
                            if res:
                                if self._queue.qsize() < self.max_queue_size: await self._queue.put(res)
                                else:                                         raise BinanceWebsocketQueueOverflow(f"Message queue size {self._queue.qsize()} exceeded maximum {self.max_queue_size}")
