...
import ipc
from managers import analyzer, binance_api, data_manager, gui_manager, neural_network_manager, simulation_manager, simulator, trade_manager
...
def managerProcess(processType, ipc_Queues, **puParams):
    ipcA = ipc.IPCAssistant(processType, ipc_Queues)
    ...
    if   (processType == 'GUI'):                  manager = gui_manager.GUIManager(path_PROJECT,                            ipcA)
    elif (processType == 'DATAMANAGER'):          manager = data_manager.DataManager(path_PROJECT,                   ipcA)
    elif (processType == 'BINANCEAPI'):           manager = binance_api.BinanceAPIManager(path_PROJECT,              ipcA)
    elif (processType == 'TRADEMANAGER'):         manager = trade_manager.TradeManager(path_PROJECT,                 ipcA, _PROCESSES_ANALYZERS)
    elif (processType == 'SIMULATIONMANAGER'):    manager = simulation_manager.SimulationManager(path_PROJECT,       ipcA, _PROCESSES_SIMULATORS)
    elif (processType == 'NEURALNETWORKMANAGER'): manager = neural_network_manager.NeuralNetworkManager(path_PROJECT, ipcA)
    elif (processType.startswith('ANALYZER')):    manager = analyzer.Analyzer(path_PROJECT,                         ipcA, int(processType[8:]))
    elif (processType.startswith('SIMULATOR')):   manager = simulator.Simulator(path_PROJECT,                       ipcA, int(processType[9:]))
...
if __name__ == "__main__":
    ...
    try:
        ...
        global ipcA
        ipcA = ipc.IPCAssistant('MAIN', ipc_Queues)
